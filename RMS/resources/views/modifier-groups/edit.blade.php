@extends('layouts.app')
@section('title', 'Editar grupo de modificadores — Don Pulpo RMS')
@section('content')
<div class="d-flex align-items-center gap-2 mb-3 mt-2">
    <a href="{{ route('modifier-groups.index') }}" class="btn btn-sm btn-outline-secondary">← Volver</a>
    <h2 class="h4 mb-0">Editar: {{ $group->name }}</h2>
</div>
...
<div class="card shadow-sm border-0" style="max-width:640px">
    <div class="card-body">
        ...
        <form action="{{ route('modifier-groups.update', $group) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select name="selection_type" class="form-select">
                    @foreach($selectionTypes as $type)
                        <option value="{{ $type }}" {{ old('selection_type', $group->selection_type) === $type ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select name="pricing_mode" id="pricingMode" class="form-select">
                    <option value="delta" {{ old('pricing_mode', $group->pricing_mode) === 'delta' ? 'selected' : '' }}>
                        Ajuste sobre el precio del platillo (ej: extra queso +$15)
                    </option>
                    <option value="absolute" {{ old('pricing_mode', $group->pricing_mode) === 'absolute' ? 'selected' : '' }}>
            ...
            <div class="mb-3 form-check">
                <input class="form-check-input" type="checkbox" name="required" id="required" value="1" {{ old('required', $group->required) ? 'checked' : '' }} />
...
const existingOptions = @json($group->options->map(fn($o) => ['id' => $o->id, 'name' => $o->name]));
if (existingOptions.length) {
    existingOptions.forEach(o => addOptionRow(o.id, o.name));
} else {
    addOptionRow();
}
