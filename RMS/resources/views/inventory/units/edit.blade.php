        ...
        <form action="{{ route('inventory.units.update', $unit) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="base_unit_id" class="form-select">
                    ...
                    @foreach($units as $u)
                        <option value="{{ $u->id }}" {{ (string) old('base_unit_id', $unit->base_unit_id) === (string) $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->abbreviation }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label class="form-label fw-semibold">Factor de conversión a la unidad base</label>
                <input type="number" step="0.0001" min="0.0001" name="conversion_factor" class="form-control" value="{{ old('conversion_factor', $unit->conversion_factor) }}" required />
            </div>
