    ...
    <div class="card-body">
        ...
        <form action="{{ route('inventory.transfers.store') }}" method="POST">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="origin_warehouse_id" class="form-select" required>
                        ...
                        @foreach($warehouses as $w)
                            <option value="{{ $w->id }}" {{ (string) old('origin_warehouse_id') === (string) $w->id ? 'selected' : '' }}>{{ $w->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <select name="destination_warehouse_id" class="form-select" required>
                        <option value="">Seleccionar…</option>
                        @foreach($warehouses as $w)
                            <option value="{{ $w->id }}" {{ (string) old('destination_warehouse_id') === (string) $w->id ? 'selected' : '' }}>{{ $w->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4 mb-3">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
                <label class="form-label fw-semibold mb-0">Productos a transferir</label>
            ...
            <button type="submit" class="btn btn-dp mt-2">Crear transferencia (borrador)</button>
            <small class="text-muted d-block mt-2">Se crea en borrador; las existencias se mueven al completarla.</small>
        </form>
    </div>
</div>
